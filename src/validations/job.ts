...
export const createJobSchema = Yup.object().shape({
    ...
    job_req: Yup.string().trim().required("Job requirements is required"),
    about_job: Yup.string().trim().required("About job is required"),
    ...
    job_keyword: Yup.array().of(Yup.string().trim().required("Requirements is required")).min(1, "At least one requirement"),
