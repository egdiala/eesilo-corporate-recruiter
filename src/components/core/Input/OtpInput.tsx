    ...
    <div className='grid gap-2 content-start'>
        <div className='flex items-center w-full gap-4'>
            {valueItems.map((item, i) => (
                ...
                {...props}
                key={i.toString()}
                className={cn("neesilo-otp-input", props.error ? "neesilo-input--border-error" : "neesilo-input--border", props.className)}
                type='text'
        ...
        <RenderIf condition={!!props?.error}>
            <span className='neesilo-input--error'>{props?.error}</span>
        </RenderIf>
    </div>
  );
};