...
export const OnboardingPage: React.FC = () => {
  ...
  const firstIncompleteStage = useMemo(() => {
    return stages?.find(
      (stage) =>
        account?.onboarding_stage?.[
          stage as keyof typeof account.onboarding_stage
        ] === false
    );
  }, [account, stages]);
  ...
  const handleNextTab = () => {
    setSelectedIndex((prevIndex) => {
      const currentIndex =
        typeof prevIndex === "string"
          ? Number.parseInt(prevIndex) || 0
          : prevIndex;
      return Math.min(currentIndex + 1, tabs.length - 1);
    });
  };
    ...
    <Fragment>
      <RenderIf condition={!isLoading}>
        <motion.div
          ...
          className="flex flex-col max-w-7xl w-full mx-auto view-page-container overflow-hidden"
        >
            ...
            <div className="flex items-start w-full">
              <div className="grid gap-1 flex-1">
                ...
                <p className="font-medium text-base text-gray-500">
                  Get started by setting up your profile. It’s quick and easy!
                </p>
              </div>
          ...
          <div className="flex-1 flex-col view-onboarding-container overflow-y-scroll scrollbar-hide py-[1.125rem] md:py-12 px-[1.125rem] md:px-12">
            ...
            >
              <TabList className="md:sticky top-0 flex flex-col h-fit gap-2 p-2.5 overflow-hidden border border-gray-200 rounded-2xl md:max-w-72 w-full">
                <div className="flex px-2 pt-1.5 pb-1 font-medium text-xs text-gray-400 uppercase">
                  Profile setup menu
                </div>
                <div className="flex md:flex-col flex-row gap-2 overflow-x-scroll scrollbar-hide">
                  {tabs.map((tab) => (
                    <Tab
                      key={tab.id}
                      className="flex whitespace-nowrap rounded-lg p-2 text-sm font-medium text-gray-500 focus:outline-none data-[selected]:bg-primary-500 data-[selected]:text-white data-[hover]:bg-gray-100 data-[hover]:text-gray-900 data-[focus]:outline-0 transition duration-500 ease-out"
                    >
                      {tab.label}
                    </Tab>
                  ))}
              ...
              <TabPanels className="max-w-xl w-full flex-1">
                ...
                <TabPanel as={AnimatePresence} mode="popLayout">
                  <ContactPerson onClick={handleNextTab} data={account} />
                </TabPanel>
                <TabPanel as={AnimatePresence} mode="popLayout">
                  <Verification
          ...
          >
            <div className="fixed inset-0 z-10 w-screen overflow-y-auto bg-gray-300/30">
              <div className="flex min-h-full items-end md:items-center justify-center p-4">
                <DialogPanel
                  transition
                  className="w-full max-w-[24.5rem] border border-gray-200 rounded-2xl bg-white backdrop-blur-2xl duration-300 ease-out transform data-[closed]:translate-y-full md:data-[closed]:translate-y-6 data-[closed]:opacity-0"
                >
                  <div className="flex flex-col items-center gap-4 p-5">
                    ...
                    <div className="grid gap-1 text-center">
                      ...
                      <p className="text-sm text-gray-500">
                        You can always update your profile in the PROFILE
